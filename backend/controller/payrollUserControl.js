...
exports.createPayrollUser = catchAysncError(async (req, res, next) => {
  ...
  const payrollUser = await PayrollUser.updateOne({empId:req.body.empId},req.body);
...
exports.createManyPayrollUser = catchAysncError(async (req, res, next) => {
  const data = req.body
  const newEmployeeErp = await PayrollUser.find({$and:[{tempPassword:{$exists:false}},{password:{$exists:false}}]},{_id:0,empId:1})
  ...
  for(let i=0;i<data.length;i++)
  {
    ...
    {
      await PayrollUser.updateOne({empId:data[i].empId},data[i]);
    }
    else
    {
      notUpdated.push(data[i])
    }
  }
...
exports.getAllPayrollUser = catchAysncError(async (req, res, next) => {
  ...
  payrollEmployee.forEach((payEmp) =>{
    ...
    {
      if(payEmp.employeeId === erpEmployee[i].empId)
      {
        erpEmp.employeeId=undefined;
        employeeData.push({basic:erpEmp,payrollData:payrollEmployee[i]})
      }
    }
...
exports.singleEmployee = async (req, res) => {
  const singleEmp = await PayrollUser.findOne({ _id: req.params.id });
...
exports.editEmployee = async (req, res) => {
  await PayrollUser.findByIdAndUpdate(req.params.id, req.body);
  res.send(req.body);
};
...
exports.createPassword = catchAysncError(async (req, res, next) => {
  ...
  if (!payrollUser) {
    ...
    return next(new ErrorHandler("This is not the first time to set password", 200));
  } else {
...
exports.userData = catchAysncError(async (req, res, next) => {
  ...
  const erpEmployee = await PayrollUser.findOne({
    employeeId: payrollUser.emp_id,
  });
  console.log(erpEmployee.email);
  // attendance database
  ...
  // specific month data of employee
    ...
    for (let m = 0; m < monthArray.length; m++) {
      const attendanceMonth =
        monthArray[m].getMonth() - 1 == -1
          ? months[11]
          : months[monthArray[m].getMonth()];
...
exports.getNewUsers = catchAysncError(async(req,res)=>{
  ...
  const data = await PayrollUser.find({$and:[{tempPassword:{$exists:false}},{password:{$exists:false}}]},{_id:0,empId:1})
